...
extension SpaceIndexResolvable {
    ...
    ///
    /// 返回 **屏内位次**（该 display 的第几个 space，1 起，按 yabai index 升序）——
    /// 角标语言 =「屏号-位次」（如 3-1 / 3-2，2026-09-09 用户裁定），与设置页
    /// minimap 胶囊的「屏号-位次」标注同一语言；屏号来自 yabai displayIndex
    /// （`OverlayWindow.calculateOverlayLabel`）。
    ///
