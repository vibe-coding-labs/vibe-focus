...
1. **命名**：对外叫什么？`vibefocus` tool 前缀已顺手；总开关文案用「Agent 接入」还是「AI 接入」？
2. **L2 的确认语义**：grid create 由 agent 直接执行（事前授权模式），
   还是要人确认？（推荐前者+快照兜底，否则无人值守流断裂。）
3. **远程 agent** 的命令通道：SSH 隧道转发 127.0.0.1，还是扩展 spool 语义承载命令？
   （影响 A4 设计，A1~A3 不受影响。）
4. **气泡唤起**：要不要给 agent `bubble summon`（让人对指定会话输入）？
   有产品价值但涉合成注入边界，建议单独立项评估。
