...
// MARK: - B216：WindowStateStore+Bindings 查询/删除/计数全分支（临时库 + db-nil 不可用库）
...
extension RunnerHarness {
    func runBindingsStoreCoverageTests() {
        do {
            ...
            check("bindings216: findWindowStateByWindowID 命中回读",
                  byWID?.windowID == 1 && byWID?.sessionID == "sess-1")
            check("bindings216: findWindowStateByWindowID 未命中 nil",
                  store.findWindowStateByWindowID(999) == nil)
            check("bindings216: findWindowStateBySession 命中",
                  store.findWindowStateBySession(sessionID: "sess-2")?.windowID == 2)
            check("bindings216: findWindowStateBySession 未命中 nil",
                  store.findWindowStateBySession(sessionID: "nope") == nil)

            // --- 计数 + 单删 + 全删 ---
            check("bindings216: windowStatesCount 计 2", store.windowStatesCount == 2)
            store.deleteWindowState(windowID: 1)
            check("bindings216: deleteWindowState 后计数 1 且查询 nil",
                  store.windowStatesCount == 1 && store.findWindowStateByWindowID(1) == nil)
            store.deleteAllWindowsStates()
            check("bindings216: deleteAllWindowsStates 清零",
        ...
        do {
            ...
            check("bindings216: 坏路径 db 为 nil", store.db == nil)
            let now = Date()
            let ws = WindowState(
            ...
            check("bindings216: 坏库 save 静默不崩", true)
            check("bindings216: 坏库三查询全 nil",
                  store.findWindowState(windowID: 1) == nil
                  && store.findWindowStateBySession(sessionID: "s") == nil
                  && store.findWindowStateByWindowID(1) == nil)
            check("bindings216: 坏库计数 0 / 全载空", store.windowStatesCount == 0 && store.loadAllWindowStates().isEmpty)
            check("bindings216: 坏库删除与清理幂等",
              { store.deleteWindowState(windowID: 1); store.deleteAllWindowsStates(); return true }() && true)
            check("bindings216: 坏库 prune 返回 0",
