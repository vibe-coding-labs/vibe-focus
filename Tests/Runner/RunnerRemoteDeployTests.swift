...
extension RunnerHarness {
    func runRemoteDeployTests() {
        ...
        let testDomain = "test.vf.b129.deploy"

        // 1. 无旗标 → nil（调用方继续正常启动）
        check("remoteDeploy: 无旗标 → nil",
              RemoteInstallDeploy.scriptForArguments(["VibeFocusHotkeys"], domain: testDomain) == nil)

        // 2. 旗标 + 显式 host + label：生成物带 host/label/派生 label 兜底
        let script = RemoteInstallDeploy.scriptForArguments(
            [flag, "10.0.0.9", "remote-lab-9"], domain: testDomain)
        ...
        let derived = RemoteInstallDeploy.scriptForArguments([flag, "10.0.0.9"], domain: testDomain)
        ...
        CFPreferencesSetAppValue("claudeHookToken" as CFString, "tok-b129" as CFString, testDomain as CFString)
        CFPreferencesSetAppValue("claudeHookPort" as CFString, 39999 as CFNumber, testDomain as CFString)
        CFPreferencesAppSynchronize(testDomain as CFString)
        let injected = RemoteInstallDeploy.scriptForArguments([flag, "10.0.0.9"], domain: testDomain)
        check("remoteDeploy: 假域 token/port 注入生成物",
              injected?.contains("tok-b129") == true && injected!.contains("39999"))
        CFPreferencesSetAppValue("claudeHookToken" as CFString, kCFNull as CFTypeRef?, testDomain as CFString)
        CFPreferencesSetAppValue("claudeHookPort" as CFString, kCFNull as CFTypeRef?, testDomain as CFString)
        CFPreferencesAppSynchronize(testDomain as CFString)

        // 5. 域为空 → token 兜底空串、port 兜底 39277（生成物仍可用）
        let empty = RemoteInstallDeploy.scriptForArguments([flag, "10.0.0.9"], domain: testDomain)
        check("remoteDeploy: 空域兜底 port=39277",
              empty?.contains("Target: 10.0.0.9:39277") == true)

        // 6. label 位置参数以 - 开头视为未提供（不吃掉后续旗标）
        let noLabel = RemoteInstallDeploy.scriptForArguments([flag, "10.0.0.9", "--verbose"], domain: testDomain)
